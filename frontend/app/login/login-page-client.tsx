...
function LoginPageContent() {
    ...
    const isAccessDenied = useMemo(() => {
        ...
        const normalized = decodeURIComponent(authError).trim().toLowerCase();
        return ["accessdenied", "access denied", "access_denied"].includes(normalized);
    }, [authError]);
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
            <Card className="w-full max-w-md">
                ...
                <CardContent className="space-y-4">
                    {isAccessDenied ? (
                        <p className="text-sm text-center">Your Google account is not authorized for this app.</p>
                    ) : (
                        <>
